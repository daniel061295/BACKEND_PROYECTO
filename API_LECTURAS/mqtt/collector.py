    ...
    if data:
        for element in data:
            ...
            try:
                ...
                # print('fechahora\t=\t{}\nnodo\t\t=\t{}\ntemperatura\t\t=\t{}\nhumedad\t\t=\t{}\n'.format(fechahora,nodo,temperatura,humedad))
                # print(Medidas.objects.filter(date_time=datetime.strptime(fechahora, '%Y-%m-%d %H:%M:%S.%f'),id_nodo=nodo))
                if list(Medidas.objects.filter(date_time=datetime.strptime(fechahora, '%Y-%m-%d %H:%M:%S.%f'),id_nodo=nodo).values_list()):
                    print("MEDICION REPETIDA, NO SE GUARDARA EN LA DB!")
                else:
                    new_medida = Medidas(date_time = datetime.strptime(fechahora, '%Y-%m-%d %H:%M:%S.%f'), id_nodo = nodo, temperatura = temperatura,
                                     humedad = humedad, id_sensor = 1)
                    new_medida.save()
                    # time.sleep(0.5)
